...
float distanceToLine(vec2 a,vec2 b,vec2 x){
  vec2 M=a-x;
  vec2 N=b-a;
  float t=-dot(M,N)/dot(N,N);
  if(t<0)return distance(a,x);
  if(t>1)return distance(b,x);
  return distance(a+t*(b-a),x);
}
...
float Substract(float a,float b){
  return Intersect(a,Neg(Intersect(a,b)));
}

vec2 warp2(vec2 x){
  x.y-=pow(abs((x-.5).x),2)*.1;
  return x;
}

vec2 coord=gl_FragCoord.xy/vec2(WindowSize);

float Circe(vec2 c,vec2 pos,float r){
  return -1+2*float(distance(c,pos)<r);
}

float Rectangle(vec2 c,vec2 a,vec2 b){
  return -1+2*float(all(greaterThan(c,a))&&all(lessThan(c,b)));
}

float Hledi(vec2 c,float w,float r,vec2 pos){
  return Union(Union(
    Circe(c,vec2(pos.x-r/2,pos.y),w),
    Circe(c,vec2(pos.x+r/2,pos.y),w)),
    Rectangle(c,pos-vec2(r/2,w),pos+vec2(r/2,w))
    );
}

float Pasek(vec2 c,float w,float r,vec2 pos,float sir){
  return Substract(
  Hledi(c,w    ,r    ,pos),
  Hledi(c,w-sir,r-sir,pos)
  );
}

vec2 Repeat(vec2 x,float k){
  return mod(x,1/k)*k;
}

#define HLEDI_W .1 // vyska
#define HLEDI_R .3 // roztec
#define HLEDI_POS vec2(.5,.75) // pozice
...
void main(){
  ...
  float background=1;
  float hledi=Hledi(coord,HLEDI_W,HLEDI_R,HLEDI_POS);
  float huba=Hledi(coord,HLEDI_W*.7,HLEDI_R*.9,HLEDI_POS-vec2(0,.25));
  float eye1=Circe(coord,HLEDI_POS-vec2(EYE_RZ,0),EYE_R);
  float eye2=Circe(coord,HLEDI_POS+vec2(EYE_RZ,0),EYE_R);
  float pasek=Pasek(coord,HLEDI_W,HLEDI_R,HLEDI_POS,.026);
  float pruh=Substract(
  Pasek(coord,HLEDI_W,HLEDI_R,HLEDI_POS,.02),
  Pasek(coord,HLEDI_W,HLEDI_R,HLEDI_POS,.01) 
  );
  float oci=Union(Substract(Union(Substract(background,hledi),Union(eye1,eye2)),pasek),pruh);
  float zuby=Rectangle(Repeat(warp2(coord),20),vec2(.01),vec2(.9));
  float usta=Neg(Intersect(Neg(Intersect(zuby,huba)),huba));
  fColor=vec4(usta);
  fColor=vec4(Intersect(usta,oci));

  return;
  //float ada=distanceToLine(vec2(.3,.62),vec2(.7,.62),warp2((coord0-.5)*vec2(1.3,1)+.5));
  //fColor=vec4(ada>.12&&ada<.14);
  //return;
  coord/=.5;
  coord=mod(coord,.5);
  ...
  float l1=max(abs(coord.x),abs(coord.y));
  float l2=length(coord);
  float cerno=float(distanceToLine(vec2(.3,.62),vec2(.7,.62),(coord0-.5)*vec2(1.3,1)+.5)>.12);
  float mask=float((l1>.1&&l2<.45&&coord0.x>.25&&coord0.y<.75&&coord0.x<.75&&coord0.y>.5))+(cerno);
  fColor=vec4(1,1,.5,1)*mask;
  return;
  float n=noise(uvec2(gl_FragCoord.xy)+0xfffff00u,4u,4u,1.7);
