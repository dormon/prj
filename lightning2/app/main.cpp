...
#define VERTICES 10000
  Lightning->use();
  Lightning->set("InverseProjectionViewMatrix",1,GL_FALSE,glm::value_ptr(InverseProjectionView));
  Lightning->set("Time",Time);
  Lightning->set("WindowSize",1,windowParam.size);
  Lightning->set("CameraPos",-Pos[0],-Pos[1],-Pos[2]);
  Lightning->set("VERTICES",VERTICES);
  Lightning->set("v",1,GL_FALSE,glm::value_ptr(View));
  Lightning->set("p",1,GL_FALSE,glm::value_ptr(Projection));

  if(Window->isKeyOn('o'))Lightning->set("alphaOn",true);
  else Lightning->set("alphaOn",false);

  if(Window->isKeyOn('l'))Lightning->set("doSwapping",true);
  else Lightning->set("doSwapping",false);
...
void Init(){
  ...
  Lightning = new ge::gl::ProgramObject(
      ShaderDir+"lightning.vp",
      ShaderDir+"lightning.gp",
      ShaderDir+"lightning.fp");
