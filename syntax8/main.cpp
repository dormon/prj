...
class Kernel{
  public:
    ...
    static void kernelTypes(ge::parser::SyntaxNode::Node,void*data){
      assert(data!=nullptr);
      auto k=(Kernel*)data;
      assert(k->typeRegister!=nullptr);
      for(size_t i=0;i<k->typeRegister->getNofTypes();++i)
        std::cout<<k->typeRegister->type2Str(i)<<std::endl;
    }
    
    static void kernelFunctions(ge::parser::SyntaxNode::Node,void*data){
      assert(data!=nullptr);
      auto k=(Kernel*)data;
      assert(k->functionRegister!=nullptr);
      std::cout<<k->functionRegister->str()<<std::endl;
    }
    ...
    static void expressionOr(ge::parser::SyntaxNode::Node node,void*d){
      ...
      assert(d!=nullptr);
      auto k=(Kernel*)d;
      auto fac = std::make_shared<ge::de::FunctionNodeFactory>("",1);
      fac->setFactory(k->functionRegister->sharedFactory("Or"));
      fac->addResourceFactory(std::make_shared<ge::de::ResourceFactory>(k->typeRegister->getTypeId("bool"),1));
      fac->addResourceFactory(std::make_shared<ge::de::ResourceFactory>(k->typeRegister->getTypeId("bool"),1));
...
int main(){
  ...
  ge::parser::Syntax syn(ge::core::loadTextFile("/home/dormon/Desktop/lex2.txt"),ge::core::loadTextFile("/home/dormon/Desktop/syn2.txt"));
  ...
  processor.addCallback("STATEMENT","statement-kernel-types",nullptr,Kernel::kernelTypes,&kernel);
  processor.addCallback("STATEMENT","statement-kernel-functions",nullptr,Kernel::kernelFunctions,&kernel);
