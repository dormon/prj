...
class Vars{
  public:
    ...
    bool        has   (std::string const&varName)const;
    Var::Type const& getType(std::string const&varName);
    Var::Stamp stamp(std::string const&varName);
    void updateStamp(std::string const&varName);
...
bool Vars::has   (std::string const&varName)const{
  return name2Var.count(varName) > 0;
}

std::type_info const& Vars::getType(std::string const&varName){
  return get(varName)->getType();
}


Var::Stamp Vars::stamp(std::string const&varName){
  return get(varName)->stamp();
}

void Vars::updateStamp(std::string const&varName){
  get(varName)->updateStamp();
}
