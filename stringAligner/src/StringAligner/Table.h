...
class STRINGALIGNER_EXPORT stringAligner::Table : public Block {
 ...
 protected:
  bool mutable needToRecomputeWidth  = true;
  bool mutable needToRecomputeHeight = true;
  size_t mutable width               = 0;
  size_t mutable height              = 0;
  ...
  bool isDecoratorThenConvertIndexToRowIndexAndDecoratorIndex(size_t& row,
                                                              size_t& decorator,
                                                              size_t& i) const;
  std::string getRowLine(size_t row, size_t line) const;
  Cell const& getCellInternal(size_t row, size_t column) const;
  void        setOnChangeCallback(std::shared_ptr<Block> const& block,
                                  size_t                        row,
                                  size_t                        column);
};
