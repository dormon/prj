...
void Cell::setData(shared_ptr<Block> const& d) { data = d; }

shared_ptr<Block> Cell::getData() const { return data; }

void Cell::setFiller(char f) { filler = f; }

char Cell::getFiller() const { return filler; }

Cell::Alignment Cell::getAlignment() const { return alignment; }

void Cell::setAlignment(Alignment a) { alignment = a; }
...
bool isHorizontalCenter(Cell::Alignment a) { return !isLeft(a) && !isRight(a); }

bool isVerticalCenter(Cell::Alignment a) { return !isBottom(a) && !isTop(a); }

void get1DPadding(size_t& a,
                  size_t& b,
                  size_t  maxSize,
                  size_t  size,
                  int     alignment)
...
{
  return -1 + static_cast<int>(isHorizontalCenter(a)) +
         2 * static_cast<int>(isRight(a));
}

int get1DVerticalAlignment(Cell::Alignment a)
{
  return -1 + static_cast<int>(isVerticalCenter(a)) +
         2 * static_cast<int>(isBottom(a));
}

Padding getPadding(Cell::Alignment a,
                   size_t          maxWidth,
                   size_t          maxHeight,
                   size_t          blockWidth,
                   size_t          blockHeight)
{
  ...
  Padding    result;
  get1DPadding(result.left, result.right, maxWidth, blockWidth,
               get1DHorizontalAlignment(a));
  get1DPadding(result.top, result.bottom, maxHeight, blockHeight,
               get1DVerticalAlignment(a));
  return result;
}

string Cell::getLine(size_t maxWidth, size_t maxHeight, size_t i) const
{
  if (!data) return string(maxWidth, filler);
  auto const blockHeight = data->getHeight();
  auto const blockWidth  = data->getWidth();
  auto const padding =
      getPadding(alignment, maxWidth, maxHeight, blockWidth, blockHeight);
