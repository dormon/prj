...
void insertToVectorAndResizeIfNecessary(vector<string>&v,size_t i,string const&d){
...
void throwIfColumnIsTooLarge(size_t nofColumns,size_t i){
  ...
  ss << "column index: " << i << " is larger than number of columns: " << nofColumns;
...
bool Table::isDecoratorThenConvertIndexToRowIndexAndDecoratorIndex(size_t&row,size_t&decorator,size_t&i)const{
  decorator = 0;
  row = 0;

  if(i==0 && getVerticalDecorator(0) != "")
    return true;
...
void throwIfLineIsOutOfRange(size_t height,size_t i){
  if(i<height)return;
  stringstream ss;
  ss << "cannot call: stringstream::Table::getLine" << "(" << i << ") - ";
...
string Table::getLine(size_t line)const{
  ...
  if(isDecoratorThenConvertIndexToRowIndexAndDecoratorIndex(row,decoratorId,line))
...
size_t computeDecoratorsLength(std::function<string(size_t)>const&getDecorator,size_t n){
  size_t counter = getDecorator(0).size();
  for(size_t i=0;i<n;++i)
    counter += getDecorator(i+1).size();
  return counter;
}

size_t computeLength(std::vector<size_t>const&sizes,std::function<string(size_t)>const&getDecorator){
  size_t counter = 0;
...
size_t Table::getWidth()const{
  if(needToRecomputeWidth){
    width = computeLength(columnWidths,[&](size_t i){return getHorizontalDecorator(i);});
...
size_t Table::getHeight()const{
  if(needToRecomputeHeight){
    height = computeLength(rowHeights,[&](size_t i){return getVerticalDecorator(i);});
...
void throwIfRowIsOutOfRange(size_t nofRows,size_t row,size_t column,string const&fce){
  if(row < nofRows)return;
  stringstream ss;
  ss << "cannot call stringAligner::Table::" << fce << "(" << row << "," << column << ")";
  ss << " - row = " << row << " >= nofRows = " << nofRows;
  throw runtime_error(ss.str());
}

void throwIfColumnIsOutOfRange(size_t nofColumns,size_t row,size_t column,string const&fce){
...
size_t computeRowHeight(std::vector<std::vector<Cell>>const&cells,size_t row){
  size_t result = 0;
  for(auto const&c:cells.at(row))
    result = std::max(result,c.getHeight());
  return result;
}

size_t computeColumnWidth(std::vector<std::vector<Cell>>const&cells,size_t column){
  size_t result = 0;
...
void clearOldBlockOnChangeCallback(Cell&cell){
  ...
  if(oldBlock)
    oldBlock->setOnChange(nullptr);
}

void Table::setOnChangeCallback(shared_ptr<Block>const&block,size_t row,size_t column){
