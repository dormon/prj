...
#define DEF_IMPL(name,number)\
  void name##_impl(int a)const{return this->gl_call(&Table::name##_impl,number,a);}
...
class Table{
  public:
    ...
    FCEPTR ptrs[4096];
    MEMBERFCEPTR memptrs[4096];
    template<typename OUTPUT,typename...ARGS>
      OUTPUT gl_call(OUTPUT(Table::*)(ARGS...)const,size_t id,ARGS...args)const{
        return ((OUTPUT(*)(ARGS...))this->ptrs[id])(args...);
      }
    void  a_impl(int a          )const{return this->gl_call(&Table::a_impl,0,a  );}
    float b_impl(float a,float b)const{return this->gl_call(&Table::b_impl,1,a,b);}
    Table(){
      this->memptrs[0]=(MEMBERFCEPTR)&Table::a_impl;
      this->memptrs[1]=(MEMBERFCEPTR)&Table::b_impl;
    }
    void  a(int val        )const{return(this->*(decltype(&Table::a)(this->memptrs[0])))(val);}
    float b(float x,float y)const{return(this->*(decltype(&Table::b)(this->memptrs[1])))(x,y);}
};
...
int main(){
  Table table;
  table.ptrs[0]=(Table::FCEPTR)(printInt);
  table.ptrs[1]=(Table::FCEPTR)(sumFloat);
  table.a(230);
  std::cout<<"FLOAT: "<<table.b(1.3f,2.f)<<std::endl;
  return 0;
}
