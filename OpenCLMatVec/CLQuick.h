...
inline CLProgram createProgram(CLBase const &clb, std::string const &src,std::string const&options = "") {
  ...
  if (result.program.build({clb.dev},options.c_str()) != CL_SUCCESS) {
...
inline CLKernel createKernel(CLBase const&clb,std::string const&kernelName,std::string const&src,std::string const&options = ""){
...
class TypedKernelParameter: public KernelParameter{
  public:
    TypedKernelParameter(cl_int parameterId,T const&parameter):KernelParameter(parameterId),parameter(parameter){}
...
void setKernelParameters(CLKernel &kernel,KernelParameters const&parameters){
  for(auto const&x:parameters)
    x.second->set(kernel);
}

template<typename T>
void addKernelParameter(KernelParameters &parameters,size_t parameterId,T const&parameter){
  parameters[parameterId] = std::make_shared<TypedKernelParameter<T>>(parameterId,parameter);
}
